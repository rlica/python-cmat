...
### Key Capabilities:
- **Full `.cmat` Reverse-Engineering**: Decompresses all proprietary IVF block compression algorithms (Modes 0–32 bit-packing, Mode 33 16-bit sparse lists, Mode 34 32-bit sparse lists, Mode 37 tagged token bit-streams, Mode 41 unary run-length shift maps) directly in pure Python + NumPy. Robustly handles symmetric, asymmetric, and arbitrary step sizes ($32 \times 32$, $32 \times 64$, $64 \times 128$, $128 \times 128$, etc.).
...
- **Interactive Web Viewer (`cmat_webviewer.py` + `cmat_webviewer.html`)**:
  ...
  - **Classic Binned Histograms**: Nuclear physics stepped staircase histograms with dynamic auto-scaling, synchronized real-time cursor highlighting across 2D and 1D views, calibrated energy readouts (keV), and ASCII `.dat` export.
    - **1D Histogram Peak Fitting (`xtrackn AG`)**: Dedicated 1D nonlinear least-squares peak fitting with linear background subtraction (`Ctrl+Click` or `V` on any 1D projection spectrum). Supports three scientific models: Standard Symmetric Gaussian, RadWare / SAMPO Piecewise Exponential Left Tail, and Hypermet Convolved Tail + $\text{erfc}$ Compton Step. Calculates centroid, net area, FWHM, and amplitude with complete statistical covariance error propagation. Multiple fits remain persistently on display simultaneously with their fit curves and baselines, adding labeled centroid markers (▼); all active fits automatically recalculate in real-time when zooming in the 2D matrix, and stay on display until cleared with `=`.
...
| **2D Coincidence Peak Fit** | `Ctrl / Cmd + Click` (2D) or `V` | True 2D coincidence peak fit (Gaussian/RadWare/Hypermet) with Gamba & Morhác 4-component BG decomposition; persistent fits remain on display and add centroid labels to both 1D projections until cleared with `=` |
| **1D Peak / Multiplet Fit** | `Ctrl / Cmd + Click` (1D) or `V` | Fit 1D peak or multiplet (constrained by background regions `B` if set; full covariance); persistent fits remain on display and add centroid labels until cleared with `=` |
...
| **Clear Peak Fits & Markers** | `=` (Equals) or `+` | Clear all active persistent peak fits (1D & 2D), fit curves, centroid labels, integration results, and peak markers |
...
| **Print 2D PDF** | `Print PDF` (2D footer) | Export publication-quality vector PDF of current 2D matrix (Times New Roman, Energy keV axes, colorbar, active 2D fit ellipses, ROI boxes, and volume callouts) |
| **Print 1D PDF** | `Print PDF` (1D header) | Export publication-quality vector PDF of active 1D spectrum (Times New Roman, Energy keV axis, stepped histogram, all active fit curves, baselines, and labeled centroids) |
...
# Load .cmat file (supports symmetric, asymmetric, and arbitrary step sizes)
reader = CMATReader("GeE-symm.cmat")

# Inspect header & compression metadata
info = reader.get_info()
print(f"Matrix: {info['filename']}, Shape: {info['shape']}, Mode: {info['matrix_mode']}")
print(f"Step: {info['step']}, Blocks: {info['blocks']}")

# Decompress to 2D NumPy array (int32)
# Array shape is (res2, res1) = (Height/Y, Width/X) in standard NumPy [y, x] indexing
matrix = reader.to_numpy()
print(f"Decompressed Shape: {matrix.shape}, Total counts: {matrix.sum():,}")

# Get total 1D projections (axis 0: Det 1 / X, axis 1: Det 2 / Y)
proj_x = reader.get_projection(axis=0)
proj_y = reader.get_projection(axis=1)

# Get 1D gated coincidence slice (channels 500 to 520 on Det 1 / X -> coincidence on Det 2 / Y)
net_spec, bg_spec, raw_spec = reader.get_gate(500, 520, axis=0)

# Export directly to ASCII (.amat)
reader.export_amat("output.amat", format_type="dense")
```

---

## Decompression Algorithm & Matrix Format Reference

[GASPware](https://github.com/csteke/GASPware) / `gsort` `.cmat` matrices are stored in an IVF (Indexed Variable Format) container consisting of 512-byte records.

### Matrix Layout & Arbitrary Step Sizes
- **Flexible Dimensions & Step Sizes**: Supports both symmetric ($res_1 = res_2$, $step_1 = step_2$) and arbitrary asymmetric matrices ($res_1 \ne res_2$, $step_1 \ne step_2$, e.g. $32 \times 64$, $64 \times 128$, or $128 \times 128$ sub-blocks).
- **Sub-Matrix Block Grid**: Divided into $ndiv_1 \times ndiv_2$ blocks of size $step_1 \times step_2$ (e.g. for a symmetric $4096 \times 4096$ matrix with $128 \times 128$ blocks: $32 \times 32$ grid with 528 stored triangular blocks; for a normal $6144 \times 6144$ matrix with $128 \times 128$ blocks: 2304 stored blocks; for an asymmetric $2048 \times 4096$ matrix with $32 \times 64$ blocks: 4096 stored blocks).
- **Dual Stored Projections**: Automatically reads stored 1D total projections directly from the IVF extra segments (Segment 2 for Det 1 / $X$, Segment 3 for Det 2 / $Y$ in asymmetric matrices) with seamless fallback to full-matrix summation if missing.

### Compression Algorithms
Each sub-block is compressed using one of 5 reverse-engineered algorithms:
